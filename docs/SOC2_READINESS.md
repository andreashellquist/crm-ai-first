...
| Encryption at rest | **Built** | `ApiKey.HashedKey` is a one-way SHA-256 hash (never needs decrypting). `SsoConnection.ClientSecret` and `WebhookSubscription.Secret` are now encrypted at rest via `ISecretProtector` (`Services/SecretProtector.cs`, wrapping ASP.NET Core's Data Protection API) — closing the gap this doc originally flagged (`auth-security-expert`'s "encrypt third-party credentials at rest" guidance). **Real, stated scope limit**: uses Data Protection's default local key ring, appropriate for this app's current single-instance posture but not a durable/shared key store — a real multi-instance production deployment needs one (Azure Key Vault, Redis, a blob store) before this holds up across instances/restarts. Database-provider-level encryption at rest (e.g. the managed Postgres provider's disk encryption) remains a reasonable additional baseline layer on top of this, not a substitute for it. |
| Vulnerability/dependency scanning in CI | **Built** | A new `dependency-scan` job in `.github/workflows/ci.yml` runs `pnpm audit` and `dotnet list package --vulnerable --include-transitive` on every push/PR (the latter always exits 0 by design even when it finds something, so the job greps its own output and fails itself if it does). `.github/dependabot.yml` adds weekly automated update PRs for all three ecosystems in this repo (npm, nuget, github-actions). Closing this also surfaced and fixed a real, live issue it was built to catch: Next.js was on 16.2.10, which has several high-severity CVEs (SSRF, DoS, a middleware bypass, unauthenticated disclosure of internal Server Function endpoints) patched in 16.2.11 — upgraded, plus `pnpm.overrides` for three other vulnerable transitive dependencies (`postcss`, `sharp`, `js-yaml`). **One accepted, documented exception**: `brace-expansion` (transitive via `eslint`'s and `openapi-typescript`'s own pinned `minimatch@3`) stays on its vulnerable version — the only patched line is a breaking rewrite incompatible with `minimatch@3`'s API (verified locally: forcing it via an override breaks `pnpm lint`), and it's dev-tooling-only with no untrusted input reaching it in this app. `pnpm audit --ignore GHSA-mh99-v99m-4gvg` documents the exception explicitly rather than silently. |
...
1. ~~**Cheap, code-only, do soon**: Dependabot/vuln scanning in CI.~~ **Closed** — see the Vulnerability/dependency scanning row above. Still open: login-attempt entries added to the audit log catalog; a written data-classification table.
2. ~~**Cheap-ish, code + a little design**: application-level encryption for `SsoConnection.ClientSecret`/`WebhookSubscription.Secret`.~~ **Closed** — see the Encryption at rest row above.
